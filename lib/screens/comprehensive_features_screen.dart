...
class ComprehensiveFeaturesScreen extends StatefulWidget {
  ...
  @override
  State<ComprehensiveFeaturesScreen> createState() => _ComprehensiveFeaturesScreenState();
}

class _ComprehensiveFeaturesScreenState extends State<ComprehensiveFeaturesScreen> {
  ...
  List<_Feature> _buildAllFeatures() => [
    // SCANNING
    _Feature(icon: Iconsax.camera,          title: 'Scan Document',   subtitle: 'Camera scanner',            color: AppColors.gold,    cat: 'scanning',  action: _scanDocument),
    _Feature(icon: Iconsax.card,            title: 'Scan ID Card',    subtitle: 'ID, license, passport',     color: AppColors.blue,    cat: 'scanning',  action: _scanIdCard),
    _Feature(icon: Iconsax.receipt,         title: 'Scan Receipt',    subtitle: 'Bills and receipts',        color: AppColors.green,   cat: 'scanning',  action: _scanReceipt),
    _Feature(icon: Iconsax.scan_barcode,    title: 'QR / Barcode',    subtitle: 'Scan codes instantly',      color: const Color(0xFF6366F1), cat: 'scanning', action: _scanQr),
    _Feature(icon: Iconsax.book,            title: 'Scan Book',       subtitle: 'Two-page book scanning',    color: AppColors.orange,  cat: 'scanning',  action: _scanBook),
    _Feature(icon: Iconsax.text_block,      title: 'Whiteboard',      subtitle: 'Capture whiteboard',        color: const Color(0xFF06B6D4), cat: 'scanning', action: _scanWhiteboard),
    _Feature(icon: Iconsax.element_3,       title: 'Scan Table',      subtitle: 'Tables & spreadsheets',     color: const Color(0xFF84CC16), cat: 'scanning', action: _scanTable),
    _Feature(icon: Iconsax.personalcard,    title: 'Scan Passport',   subtitle: 'Passport scanner',          color: const Color(0xFFF43F5E), cat: 'scanning', action: _scanPassport),
    _Feature(icon: Iconsax.gallery,         title: 'Import Photo',    subtitle: 'Pick from gallery',         color: AppColors.red,     cat: 'scanning',  action: _pickAndEdit),

    // EDITING
    _Feature(icon: Iconsax.magicpen,        title: 'Auto Enhance',    subtitle: 'Improve scan quality',      color: AppColors.gold,    cat: 'editing',   action: _autoEnhance),
    _Feature(icon: Iconsax.setting_2,       title: 'Advanced Filters',subtitle: 'Brightness, contrast',     color: AppColors.blue,    cat: 'editing',   action: _advancedFilters),
    _Feature(icon: Iconsax.crop,            title: 'Crop & Edit',     subtitle: 'Crop and adjust pages',     color: AppColors.purple,  cat: 'editing',   action: _cropEdit),
    _Feature(icon: Iconsax.rotate_left,     title: 'Rotate',          subtitle: 'Rotate 90° / 180°',        color: AppColors.blue,    cat: 'editing',   action: _cropEdit),
    _Feature(icon: Iconsax.shield_tick,     title: 'Watermark',       subtitle: 'Add text watermark',        color: AppColors.orange,  cat: 'editing',   action: _watermark),
    _Feature(icon: Iconsax.pen_add,         title: 'Annotate',        subtitle: 'Draw and mark docs',        color: AppColors.blue,    cat: 'editing',   action: _annotate),
    _Feature(icon: Iconsax.clock,           title: 'Timestamp',       subtitle: 'Add date & time stamp',     color: AppColors.navyMid, cat: 'editing',   action: _cropEdit),
    _Feature(icon: Iconsax.copy,            title: 'Batch Process',   subtitle: 'Apply to multiple files',   color: AppColors.red,     cat: 'editing',   action: _batchProcess),
    ...
    // DOCUMENTS
    _Feature(icon: Iconsax.folder_open,     title: 'My Documents',    subtitle: 'All saved documents',       color: AppColors.navyMid, cat: 'documents', action: _myDocuments),
    _Feature(icon: Iconsax.import_2,        title: 'Import Files',    subtitle: 'Import from device',        color: AppColors.blue,    cat: 'documents', action: _importFiles),
    _Feature(icon: Iconsax.search_normal,   title: 'Search',          subtitle: 'Find documents fast',       color: AppColors.navyMid, cat: 'documents', action: _search),
    _Feature(icon: Iconsax.document,        title: 'Templates',       subtitle: 'Pre-made formats',          color: AppColors.green,   cat: 'documents', action: _templates),
    ...
    // SHARING
    _Feature(icon: Iconsax.share,           title: 'Share',           subtitle: 'WhatsApp, email, Drive',    color: AppColors.navyMid, cat: 'sharing',   action: _share),
    _Feature(icon: Iconsax.printer,         title: 'Print',           subtitle: 'Print documents',           color: AppColors.navyDark,cat: 'sharing',   action: _print),

    // SECURITY
    _Feature(icon: Iconsax.lock,            title: 'Biometric Lock',  subtitle: 'Fingerprint/Face lock',     color: AppColors.red,     cat: 'security',  action: _biometric),

    // TOOLS
    _Feature(icon: Iconsax.camera,          title: 'ID Photo Maker',  subtitle: 'Passport & visa photos',    color: AppColors.purple,  cat: 'tools',     action: _idPhoto),
    _Feature(icon: Iconsax.notification,    title: 'Notifications',   subtitle: 'Document reminders',        color: AppColors.gold,    cat: 'tools',     action: _notifications),
  ];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('All Features',
            style: GoogleFonts.nunito(fontWeight: FontWeight.w800, color: Colors.white, fontSize: 18)),
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isActive ? AppColors.gold : Colors.white.withOpacity(0.12),
  ...
  Future<void> _pickAndEdit() async {
    ...
    _go(EditScanScreen(imagePaths: images.map((e) => e.path).toList(), scanType: 'photo'));
  }
  ...
  Future<void> _batchProcess() async {
    ...
    if (images.isEmpty || !mounted) return;
    _go(EditScanScreen(imagePaths: images.map((e) => e.path).toList(), scanType: 'document'));
  }
  ...
  Future<DocumentModel?> _pickDoc() async {
    ...
    if (docs.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Pehle koi document scan ya import karein', style: GoogleFonts.nunito()),
        backgroundColor: AppColors.navyMid, behavior: SnackBarBehavior.floating,
      ));
      return null;
    }
          ...
          SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, i) => ListTile(
                ...
                onTap: () { selected = docs[i]; Navigator.pop(ctx); },
              ),
            ),
          ),
...
class _Feature {
  ...
  const _Feature({
    ...
    required this.color, required this.cat, required this.action,
  });
}
